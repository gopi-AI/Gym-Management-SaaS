...
### 3.3 `REPORTS_MATERIALIZED_VIEWS` (in the DB plan as an ERD node only — Phase 6 creates it, and `1788965263255-CreateMaterializedViewsTable.ts` is where that happens)

This table appears in `docs/database-plan.md` **only as a Mermaid ERD node** — that file contains no SQL DDL for it. Phase 6 **adopts** it as the registry of materialized-view definitions that §4.3's endpoints and §7.4's refresh flow operate against, and **creates it** — `src/migrations/1788965263255-CreateMaterializedViewsTable.ts` — so its physical name is the quoted `"REPORTS_MATERIALIZED_VIEWS"` form below, and the table exists in any database whose migration chain has been run through that migration. It is reproduced here so §3 is self-contained.
...
>
> **The table now exists, so the dependency above is discharged**: `FINANCE_REFUNDS` was created by Phase 6.0's own schema work (`1788965263256-CreateFinanceRefundsTable.ts`, P6-01), not by P3-02 — whose Database changes had claimed the `refunds` table was "completed in P1-04" and have since been corrected. The entity carries `refund_date`, `amount`, `reason` and `organization_id`, which is exactly this row's requirement (:469-471), so the row is seedable from **deployed metadata** rather than by decision. What P3-02 still owns is the refund **lifecycle** — the service, the API and any columns beyond that minimum — not the table.
>
