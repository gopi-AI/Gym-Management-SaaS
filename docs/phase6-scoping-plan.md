...
| **Workouts** (`workouts/`) | `WorkoutSession`, `WorkoutSessionExercise`, `WorkoutPlanAssignment` | `organization_id`, `member_id` | session_date, duration_minutes, exercises logged, template_id (WorkoutSession.template_id — resolved on **circumstantial** evidence: the two dimensions before it are WorkoutSession columns, and `plan_template` named no column anywhere; revisit if new evidence points at WorkoutPlanAssignment.template_id instead) | total sessions, avg duration, exercise adherence, trend |
| **Diet** (`diet/`) | `NutritionLog`, `DietPlanAssignment` | `organization_id`, `member_id` | log_date, meal_template_id, macro values (calories, protein_g, carbs_g, fat_g) | daily avg macros, meals logged, missing-macro ratio, plan adherence |
| **PT** (`pt/`) | `PTEnrollment`, `PTSession`, `TrainerCommission` | `organization_id`, `branch_id` | trainer_id, session_date (derived at read time from scheduled_start/actual_start — no PT entity stores it), status, price, currency | session count, commission earned, trainer utilization, revenue |
...
> **Note — §1.2's Finance, Memberships and PT rows, and where the preamble now stands (explicit decision)**: the Finance row listed `Refund` and `PaymentAllocation` under a preamble asserting that every module here has **TypeORM entities + migrations** already deployed. `Refund` satisfies it — it ships with the finance schema as **P3-02's** deliverable, created by `1788965263258-CreateRefundAndCreditNoteTables.ts` on `main` alongside `FINANCE_CREDIT_NOTES`, which is also what makes §6.2's `Refund Report` row resolvable. Phase 6 does **not** create it: this branch briefly carried its own competing `CreateFinanceRefundsTable` migration and a catalog-minimum `Refund` entity, and both were removed because `main` already shipped both under a colliding timestamp (§6.2, :475). `PaymentAllocation` is the row's other casualty and a different case — it never had an entity or a table anywhere, and none was ever scheduled, so P6-24 removed it from the row. The Memberships row's `plan_name` is corrected to `plan_id` for the same reason: the name lives on `MembershipPlan` and is resolved at read time, not stored. The PT row's `PtEnrollment` and `PtSession` were case mismatches against the declared `PTEnrollment` and `PTSession` — found while closing P6-24, filed separately rather than folded into it, and corrected by P6-31, which fixed both spellings here and in the three §6.6 catalog rows that named `PtSession` as their source. With those corrected the preamble is true of every entity name in this table, and the §6.6 rows that source a PT entity now resolve against entity metadata.
...
### 3.3 `REPORTS_MATERIALIZED_VIEWS` (in the DB plan as an ERD node only — Phase 6 creates it, and `1788965263402-CreateMaterializedViewsTable.ts` is where that happens)

This table appears in `docs/database-plan.md` **only as a Mermaid ERD node** — that file contains no SQL DDL for it. Phase 6 **adopts** it as the registry of materialized-view definitions that §4.3's endpoints and §7.4's refresh flow operate against, and **creates it** — `src/migrations/1788965263402-CreateMaterializedViewsTable.ts` — so its physical name is the quoted `"REPORTS_MATERIALIZED_VIEWS"` form below, and the table exists in any database whose migration chain has been run through that migration. It is reproduced here so §3 is self-contained.
...
> **The migration exists — this is the shape it must keep matching**: the shape above is a hand-translation of the `REPORTS_MATERIALIZED_VIEWS` node in `docs/database-plan.md`'s **Mermaid ERD** (that file contains no SQL for it — the node sits inside a ```` ```mermaid ```` fence). `src/migrations/1788965263402-CreateMaterializedViewsTable.ts` creates the table under the quoted `"REPORTS_MATERIALIZED_VIEWS"` form above, and `grep -rln 'REPORTS_MATERIALIZED_VIEWS' src/` returns that migration and `src/reports/entities/materialized-view.entity.ts`. §7.4's registration step therefore has a table to write to wherever migrations have been run. The quoted name matches the project's convention: of **55** `CREATE TABLE` statements in `src/migrations/`, **53** quote an `UPPER_SNAKE` name and the two exceptions are the `shared` schema's lower-case `"shared"."outbox"` and `"shared"."inbox"`; among `@Entity()` declarations the split is identical, **53 of 55** `UPPER_SNAKE` with the same two `shared` exceptions.
...
> **Definition — "Member Demographics" derives age groups from the raw date of birth at read time (explicit decision)**: `age_group` is not a stored `Member` column and the current `QueryDefinition` contract does not provide an age-range or arithmetic expression variant. The definition therefore declares only the source fields and supported aggregate: raw `date_of_birth`, `gender`, `branch_id`, and `COUNT(*)`, grouped by the selected output aliases as required. This is the same derived-scalar treatment already established for `days_overdue`, `tenure_months`, `ww_change`, and the other values that are not executor expressions; adding age-bucketing syntax to the executor would be a contract extension for one catalog row.
>
> **Decision**: the read layer derives `age_group` from `date_of_birth` using the report's age-range policy. Because `Member.date_of_birth` is nullable, a member with no date of birth is assigned to an explicit `unknown` age-group bucket rather than being silently excluded from the demographic result. This preserves the count of members represented by the report while keeping `unknown` distinct from every calculated age range. `age_group` is therefore added to the derived-scalars list and is not placed in the `QueryDefinition` as an arithmetic expression.

> **Definition — "New Members" derives cumulative totals as a read-layer running sum (explicit decision)**: the catalog's `cumulative` value is not a stored `Member` column and is not a supported `ColumnRef` shape. It is also a calculation over the ordered rows produced by the monthly, weekly, or daily grouped result, whereas a single `QueryDefinition` declares one source and one grouped result set and does not express a window function or a second query over that result. Treating it as an executor expression would therefore extend the contract beyond the existing allowlist.
>
> **Decision**: the `QueryDefinition` uses the already-established executable shape — a time bucket over `created_at` (for example, the confirmed monthly `month` bucket), `COUNT(*)`, the corresponding `group_by` and `order_by` aliases, and the existing row limit where applicable. The read layer orders the returned rows chronologically and computes `cumulative` as a running sum of the per-period `count` values. `cumulative` is therefore a derived scalar, not a `QueryDefinition` column, and the existing monthly example remains the canonical definition shape.
...
>
> **The catalog now constrains the entity's own vocabulary**: `ReportExecutorService` validates a row's declared columns against current entity metadata at execution time (§10), so `Refund` must carry `refund_date` (not merely a generic `created_at`), `amount`, and `reason`, under those names. Beyond those, the row needs only `organization_id`, which the executor's tenant scoping filters on. Everything else in the entity's design (a link to the refunded invoice or payment, a refund lifecycle status, a currency, index choices) is P3-02's decision and is not required by this row.
>
...
>
> **`FINANCE_REFUNDS` is P3-02's table, and Phase 6 no longer creates a competing one (explicit decision)**: the table and the `Refund` entity are created by **P3-02** on `main` — `1788965263258-CreateRefundAndCreditNoteTables.ts`, which creates `FINANCE_REFUNDS` and `FINANCE_CREDIT_NOTES` together — not by Phase 6. An earlier revision of this branch carried its own `CreateFinanceRefundsTable` migration and a catalog-minimum `Refund` entity; **both have been removed.** They had to be: the migration sat at `1788965263256`, a timestamp `main` had already taken for `AddMemberTaxExemption`, and two `CREATE TABLE "FINANCE_REFUNDS"` statements would have left migration ordering ambiguous at that point on merge while giving one entity two competing definitions. P3-02's table is a strict **superset** of this row's requirement — it carries `refund_date`, `amount`, `reason` and `organization_id` under exactly those names (:469-471), plus the `payment_id` link and refund `status` that :471 assigns to P3-02 — so the row's declared source and columns are unaffected and it needs no columns of its own added. The dependency is discharged **by P3-02 shipping the table**, not by Phase 6 building a parallel one; the one operational consequence is that P3-02's migration lives on `main` and this branch forked before it, so on this branch alone the `Refund` source does not yet resolve and the row is seedable once `main` is merged in, not before.
>
...
>
> **Decision**: the report is redefined over `AttendanceRecord` as the smallest single-source definition the current contract can execute — it omits `group_by` and returns raw `check_in_time` rows (plus `branch_id` if the read layer needs it), constrained by the existing `date_range` and `branch` filters. The read layer buckets every returned timestamp into `hour_of_day` and computes `count`, `avg_count`, and `peak_count` from the actual returned row volume. `hour_of_day`, `avg_count`, and `peak_count` are therefore derived scalars, not `columns` expressions or nested aggregates in the `QueryDefinition`. A day bucket alone is not sufficient because it would discard the timestamp information needed to calculate hour-of-day results; retaining the raw timestamp is the smallest executable shape that preserves the required input data without widening the contract. Because `date_range` does not impose an automatic row limit, callers are expected to use a bounded range such as a single day or week; an explicit `limit` must not truncate the raw rows unless the statistics are correspondingly defined as limited-sample results.
...
| **Active Loyalty Accounts** | Distinct accounts with at least one points transaction in the period | `LoyaltyTransaction` | period, count (`COUNT(DISTINCT account_id)`) | date_range |

> **Definition — "Active Loyalty Accounts" is redefined over `LoyaltyTransaction`, and "active" now has an explicit definition (explicit decision)**: the row declared `Source = LoyaltyAccount` with Key Columns `status, count`, and `LoyaltyAccount` has **no `status` column** — its full column set is `id, organization_id, member_id, balance, lifetime_points_earned, lifetime_points_redeemed, tier, created_at, updated_at`, and `tier` is nullable and unused. The row also had a **time-axis mismatch**: its Description was "Account activity over time" with a `date_range` filter, but an account row carries only `created_at`/`updated_at` and no per-period activity fact, so it could not have produced a series even if a `status` column had been added.
>
> **Decision**: the report is redefined as a single-source query over `LoyaltyTransaction` — **an account is "active" in a period if it has at least one transaction in that period** — expressed as `COUNT(DISTINCT account_id)` grouped by period, with `date_range` binding to `created_at`. Stating the threshold is the point: a reader may now disagree with the definition (one transaction, versus some points threshold) but cannot be misled about what the number counts, which is what the original row left open. The ledger is also the right source on its own terms — it is append-only and every point movement is a row, so "activity" is exactly what it records.
>
> **This row required a schema change, and it is the one thing §6.7 was missing**: `LoyaltyTransaction` had **no `organization_id` column** (`id, account_id, transaction_type, points, remaining_points, reference_type, reference_id, description, expires_at, created_at`), so a `LoyaltyTransaction`-sourced row had no column for the executor's automatic tenant filter to bind to (§3.1.1), and `QueryDefinition` has no `joins` key through which the organization could be reached via `LoyaltyAccount`. `organization_id` is therefore **added to `LOYALTY_TRANSACTIONS`** (P6-25, resolution D): written by both of the table's existing writers — the accrual path already holds the organization, and the expiry sweep already resolves it — and backfilled for existing rows **deterministically** through `account_id` → `LOYALTY_ACCOUNTS.organization_id` before the column is made `NOT NULL`.
>
> **The scoping fix is section-wide, not row-local**: the missing `organization_id` blocked **every** `LoyaltyTransaction`-sourced row, so adding it also unblocks §6.7's other two rows (`Points Issued/Burned`, `Redemption Rate`), which carry the same single-source restriction. They are **deliberately not redefined here** — only this row is in scope, and each has its own question (`period`, `issued`, `redeemed` and `rate` are derived values, not stored columns) — but they are no longer blocked by scoping, and whoever picks them up starts from a source the executor can actually filter.
>
> **What was rejected**: (C), a persisted, maintained `status` column on `LoyaltyAccount`. Nothing in the module maintains account status and no rule defines "inactive" — points expire, accounts do not — so it would need a new worker **and** a new business rule; and a stored value is a *current* fact, not the per-period series this row describes, so (C) would have forced a snapshot redefinition as well (the §6.1 pattern) and left the figure dependent on state that can drift from the ledger. (A) and (B) from the ticket remain available but are excluded by the decision to keep a schema-backed loyalty metric rather than approximate it from memberships or drop the row.
...
> **Definition — the "Budget Utilization" row is deleted from the catalog, and its measure is served by the existing AI-usage endpoint instead (explicit decision)**: the row declared `Source = AiUsageService budget state` with Key Columns `period, budget, consumed, remaining_pct`. `AiUsageService` is a NestJS service, not a TypeORM entity — it is not in the entity registry, so it cannot be a `QueryDefinition.source` (§3.1.1) — and `ReportExecutorService` validates a row's source against current entity metadata at execution time (§10). The row was therefore unusable as a report regardless of which columns it named.
>
> **Its three "budget" values were also not one source, and one of them is not per-organization data**: `budget` came from `AiUsageLimitService.resolveLimits()`, which reads the **process-level** environment variables `AI_RATE_LIMIT_TPD` and `AI_COST_LIMIT_MONTHLY_USD`; `consumed` came from **Redis counters** keyed `ai:budget:{organizationId}:{YYYY-MM-DD}` (daily tokens) and `ai:cost:{organizationId}:{YYYY-MM}` (monthly cost) — fast, expiring counters, not durable rows; and `remaining_pct` was arithmetic over the two, computed at read time. `period` is not a column of anything, and the row could not represent both budgets at once: the daily token budget and the monthly cost budget are different counters over different windows, so a single row has no dimension to hold both.
>
> **Decision: the row is deleted rather than redefined**, because no single-source shape exists to redefine it into. This follows §6.2's treatment of the `Refund Report` row — excluding an unexecutable row rather than rewriting it — with one deliberate difference: that row is **kept** because refund data is a tracked future deliverable (P3-02), so it becomes seedable the day a `Refund` entity ships, whereas no tracked deliverable would make a *budget* row seedable and the measure already has a shipped, non-report path. Deleting it also enforces this section's own rule that no row declare a source the executor cannot resolve.
>
> **Where "budget consumed vs. remaining" lives instead**: `GET /v1/organizations/{orgId}/ai/usage` already returns exactly this measure per organization. Its `quota` object carries `tokens_used_today`, `token_limit_per_day`, `tokens_remaining_today`, `cost_used_this_month_usd`, `cost_limit_per_month_usd` and `cost_remaining_this_month_usd` (src/ai/dto/ai-usage-response.dto.ts:48-55). The AI-usage dashboard (P6-12) reads that endpoint; no catalog row, report job or export path is needed for it. (The shorter form `GET /v1/ai/usage` also appears in this document and in the backlog; the full route above is the one actually registered — `@Controller('v1/organizations')` + `@Get(':orgId/ai/usage')`.)
>
> Two shape facts a consumer must respect: `tokens_remaining_today` and `cost_remaining_this_month_usd` are **`null` when the corresponding limit is disabled** (`0`), which must render as "no budget configured" rather than "budget exhausted"; and the endpoint's figures are a **current snapshot**, not a series.
>
> **What is genuinely lost**: the ability to run this measure as a **report job** — to schedule it, filter it by `date_range` across arbitrary past periods, or export it beside the other §6 reports. Any of those would need a persisted per-organization budget table, which does not exist and is outside Phase 6.0's committed data model. §1.2's AI Usage row and §1.1's `AiUsageService` line are **unaffected**: both say AI consumption including budget remaining is *reportable*, and it is — through the endpoint. Neither claims it is a catalog row.
>
> **The ceiling reported is process-wide, not per-tenant**: `resolveLimits()` takes **no organization argument** (src/ai/services/ai-usage-limit.service.ts:249), so the limit the endpoint reports is the single configured ceiling for the process, while `consumed` is counted per organization. One tenant's AI cost ceiling is therefore every tenant's ceiling today. The counters are already keyed per organization, so making the ceiling per-tenant is a change to where the limit is read, not to how usage is counted. Recorded as **low priority, not urgent** in the P6-22 ticket; not changed here.
...
// (§6.2) included — its source entity `Refund` ships with P3-02's refund
// migration (`1788965263258-CreateRefundAndCreditNoteTables.ts`, on `main`),
// which Phase 6 does not duplicate (§6.2 :475). ReportExecutorService validates
// source and columns against entity metadata (§3.1.1, §13), so the seed set is
// exactly the set of rows that resolve. Run this migration AFTER P3-02's refund
// migration: a row whose source does not exist yet passes creation, then fails.
```
