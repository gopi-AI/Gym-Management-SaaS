...
- **Defect detail**:
  - §7.2's view casts a `timestamptz` straight to `date`: `check_in_time::date AS date,` (docs/phase6-scoping-plan.md:546) and `GROUP BY organization_id, branch_id, check_in_time::date;` (docs/phase6-scoping-plan.md:552). The source column is `TIMESTAMP WITH TIME ZONE` (src/migrations/1788965263233-CreateAttendanceSchema.ts:69; `@Column({ type: 'timestamptz' })`, src/attendance/entities/attendance-record.entity.ts:49-50).
