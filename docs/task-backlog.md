...
### P6-34: §3.3 describes `REPORTS_MATERIALIZED_VIEWS` as unbuilt, but the migration exists and the name is defined *(Open — defect)*
...
- **Acceptance criteria**:
  - §3.3 states that the table is created by a named migration, so its physical name is defined rather than pending.
  - §3.3 does not assert, as a repository-level fact, that no migration creates the table.
  - Any statement about the table's presence in a particular database is tied to that database's migration state rather than asserted unconditionally.
- **Defect detail**:
...
- **Risks**: A reader scoping the reports work from §3.3 concludes the registry table still has to be created and writes a duplicate migration; or, reading the entity's citation, trusts a section whose opening says the table does not exist.

### P6-35: Four §1.2 "Reportable Dimensions" name no column even module-wide, and each needs a decision *(Open — defect)*
...
- **Acceptance criteria**:
  - Each of the four terms either names a real column on an entity in its row's module, or is explicitly marked as derived.
  - No term is replaced by a name that merely resembles it: every replacement is verified against entity metadata, and `session_date`-style cases are decided as derived-versus-stored rather than settled by picking the nearest-looking column.
  - `date_range`'s status is settled explicitly — a dimension of the Finance row, or a filter concept filed under the wrong heading — rather than left as it is.
- **Defect detail**:
  - **The four terms, with the evidence and the proposal for each. None is applied here: three need a decision about which entity (or whether the value is stored at all), and the fourth needs a classification question answered first.**
    - **`date_range`** (Finance (:30)) — absent from all five finance entities. Nearest real columns are `Invoice.invoice_date`/`due_date`, `Payment.payment_date`, `Refund.refund_date`. It also appears **33 times in the plan as a Filter** on §6.x catalog rows, where it is plainly a filter concept rather than a column, so the Finance row's use may be that same idea misfiled under Dimensions. **Proposal:** reclassify — either drop it from the row (it is not a dimension) or replace it with the concrete date column the report in question means. That column differs per §6.1 row, so it is **not a single substitution**. **Read the §6.1 rows' actual `date_range` usage before ruling**, rather than deciding from the occurrence count alone.
    - **`plan_template`** (Workouts (:32)) — absent from all six workouts entities. Nearest real columns: `WorkoutPlanAssignment.template_id`, `WorkoutSession.template_id`, `WorkoutTemplateExercise.template_id`/`weight_template`. **Proposal:** `template_id` is the likely intent, but two entities carry that name and the dimension means *which* template, so the honest fix is `template_id` plus a note that template identity lives on `WorkoutTemplate`. Two candidates for one name is why this is a decision rather than a swap.
    - **`meal_template`** (Diet (:33)) — absent from all four diet entities. `NutritionLog.meal_template_id` exists, and there is also a `MealTemplate` entity; bare `meal_template` matches neither the column nor the entity name as written. **Proposal:** `meal_template_id`, matching the FK that exists and the convention `plan_template` should take.
    - **`session_date`** (PT (:34)) — absent from all five PT entities. `PTSession` carries `scheduled_start`/`scheduled_end`/`actual_start`/`actual_end`; `PTEnrollment` carries `start_date`/`end_date`. **Proposal:** none is obviously right. `session_date` may be intended as a derived label over `scheduled_start` (a *session's* date rather than a stored column) — the same derived-versus-stored question P6-24 answered for `plan_name`. **This is the term whose honest resolution may be "mark it derived" rather than name a column**, and choosing a column because it looks close is the failure this ticket exists to avoid.
...
- **Risks**: Four rows keep directing report authors at columns that do not exist, and the Finance row keeps a filter concept filed as a dimension. Both are the failure mode P6-30's Risks section describes, scoped to the rows P6-30 and P6-31 did not close.
