...
### P6-38: §6.3's "Avg Session Duration" needs a duration that exists only as an expression the allowlist forbids — and its own description names §6.4's measure *(Needs a platform-level scoping decision before implementation — not resolvable as a contained P6-38-only fix)*
...
- **Re-scoping addendum (2026-09-25)**: The measure is decided to mean **attendance check-in/check-out duration** from `AttendanceRecord`, matching the duration already computed by §7.2's `reports_mv_daily_attendance`. The row's "Average workout duration" description was a **mislabeling**, not a deliberate duplicate of §6.4's separate "Avg Workout Duration" report.
  - **Neither investigated path is a same-day fix.** Supporting the existing MV as a report source requires a new source-type path in the validator and builder, which currently resolve sources through TypeORM entity metadata. The MV is also daily and branch-level, excludes incomplete visits, and does not expose the row's current member-level dimension. Using it therefore entails redefining what the report shows, not just wiring up a source.
  - **A DSL arithmetic extension is shared platform work.** It could address P6-38 and P6-39's shared two-column-expression limitation, but requires design for expression grammar, null/incomplete-row handling, and duration units/result semantics (interval versus minutes versus integer). It also reopens the allowlist's security surface that `query-definition.ts` explicitly warns against loosening. P6-43 records the precedent of deliberately not widening the DSL for its PT report.
  - **Status:** Needs a platform-level scoping decision before implementation; not resolvable as a contained P6-38-only fix.
...
- **Re-measurement addendum (2026-09-25)**: The “seeded row” framing above is incorrect. “Trainer Utilization” was never seeded: it is absent from both the live system-report catalog and the report seed migration. The only PT report present there is “Completed PT Sessions by Trainer.”
  - **Three blockers prevent implementing this as a report:**
    1. **`available_hours` has no source.** There is no trainer availability, schedule, or capacity entity anywhere in the codebase. The PT migration explicitly deferred `PT_TRAINER_AVAILABILITY` and `PT_TRAINER_BOOKINGS` pending a business-rule decision.
    2. **`booked_hours` needs unsupported duration arithmetic.** Deriving it from start and end timestamps hits the same two-column arithmetic limitation recorded by P6-38/P6-39. If based on actual timestamps, it also encounters P6-47’s behavior: omitted `actual_end` defaults to `actual_start`, so the resulting duration can be zero rather than measured.
    3. **`pct` needs an unsupported aggregate ratio.** It requires calculating a ratio between two aggregate outputs, a DSL capability gap not previously identified by P6-38 or P6-39.
