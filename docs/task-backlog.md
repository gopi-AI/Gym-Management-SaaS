...
### P6-33: Five §1.2 "Reportable Dimensions" name columns that their row's own declared entities do not have *(Open — defect)*
- **Objective**: Make §1.2's "Reportable Dimensions" column true of each row's declared entities — by naming the real column, or by marking the entry as derived — the standard P6-30 and P6-31 applied to their own rows.
...
- **Files/modules affected**:
  - docs/phase6-scoping-plan.md (§1.2 Finance row (:30); Workouts row (:32); Diet row (:33); PT row (:34) — and the PT row's "Key Entities" cell, if the `price` question is resolved that way)
- **Database changes**: None
- **API changes**: None
- **Frontend changes**: None
- **Worker changes**: None
...
- **Acceptance criteria**:
  - Every term in §1.2's "Reportable Dimensions" column either exists on one of its row's declared entities under that name, or is explicitly marked as derived.
  - Every replacement is a real column name verified against entity metadata, not one that merely resembles the old name.
  - The `price` question is settled explicitly (see Defect detail), not left implicit — either the PT row gains the entity that carries it, or the column's standard is recorded as per-module rather than per-row.
- **Defect detail**:
...
- **Risks**: The column stays partly unverifiable, so "these fields are reportable" remains an assertion rather than a check — the gap P6-30 and P6-31 each closed for their own rows, left open for the rest.
