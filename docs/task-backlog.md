...
- **Dependencies**: P6-03 (creates the views) and P6-15; the §3.3 shape is the plan's, and its migration shipped with the rest of Phase 6.0's schema (`1788965263255-CreateMaterializedViewsTable.ts`), so the table exists and its shape is fixed in code rather than open.
...
- **Acceptance criteria**:
  - §3.3 states that the table is created by a named migration, so its physical name is defined rather than pending. ✅ **Met** — both the heading (:274) and the paragraph (:276) name `1788965263255-CreateMaterializedViewsTable.ts`.
...
- **Defect detail**:
  ...
  - **The migration exists, added by `aeae567d`.** `src/migrations/1788965263255-CreateMaterializedViewsTable.ts` runs `CREATE TABLE "REPORTS_MATERIALIZED_VIEWS" (id uuid DEFAULT gen_random_uuid() PRIMARY KEY, name varchar(200) NOT NULL, description text, last_refreshed timestamptz)` — exactly §3.3's shape (:279-284), under exactly the quoted name §3.3 requires. `grep -rln 'REPORTS_MATERIALIZED_VIEWS' src/` therefore returns **two** files — that migration and `src/reports/entities/materialized-view.entity.ts` — not "nothing". The name is defined, not "undefined until a migration is written".
  - **The "does not exist in the database today" clause needs a different form, not deletion.** It is true of the local dev database, measured rather than assumed: `127.0.0.1:5433/gym_management` reports **28** public tables, none matching `REPORTS%` or `%MATERIALIZED%`; `typeorm_migrations` holds **10** applied rows with `ProvisionFinancePermissions1788965263236` newest, while `src/migrations/` contains **31** files and `1788965263255` is **not** applied. So the table is absent there because that database has not run the reports migrations — a fact about one database's migration state, not the design fact §3.3 asserts. Corrected, the sentence should say what determines presence: the table exists in any database whose migration chain has been run through `1788965263255`.
...
### P6-35: Four §1.2 "Reportable Dimensions" name no column even module-wide — two resolved, two await a ruling *(Applied — partial)*
...
- **Acceptance criteria**:
  - Each of the four terms either names a real column on an entity in its row's module, or is explicitly marked as derived. ⚠️ **Met for two of four.** `meal_template_id` names a real `NutritionLog` column; `session_date` is explicitly marked derived in the PT row. `date_range` and `plan_template` remain as written, each with a proposed ruling recorded below rather than applied.
  - No term is replaced by a name that merely resembles it: every replacement is verified against entity metadata, and `session_date`-style cases are decided as derived-versus-stored rather than settled by picking the nearest-looking column. ✅ **Met** — the one replacement (`meal_template_id`) was verified against a fresh metadata dump, and the one derived-versus-stored case was resolved as derived rather than by substituting `scheduled_start`, which merely resembles it. Nothing was applied for the two open terms, which is the same criterion holding in the negative.
  - `date_range`'s status is settled explicitly — a dimension of the Finance row, or a filter concept filed under the wrong heading — rather than left as it is. ⚠️ **Not met — investigated, ruling proposed, awaiting a decision.** The read-first step this criterion implied has been completed in full (all six §6.1/§6.2 rows, quoted below): `date_range` appears only in Filters columns and never in Key Columns, and §6.1 (:420) treats it as a filter whose honoring changes query shape. The proposed ruling — drop it from the Finance row's Dimensions — is recorded and deliberately not applied.
- **Defect detail**:
  - **The four terms, with the evidence and the outcome for each. `meal_template` and `session_date` are resolved below; `date_range` and `plan_template` were investigated and are **not** applied — each carries a proposal and waits, because a wrong guess here would create a new naming defect rather than remove one.**
    - **`date_range`** (Finance (:30)) — absent from all five finance entities. Nearest real columns are `Invoice.invoice_date`/`due_date`, `Payment.payment_date`, `Refund.refund_date`. It also appears **33 times in the plan as a Filter** on §6.x catalog rows, where it is plainly a filter concept rather than a column, so the Finance row's use may be that same idea misfiled under Dimensions. **Proposal: still open — the read-first step this ticket demanded has now been done, and the finding is below. Do not apply without reading it.**
    - **`plan_template`** (Workouts (:32)) — absent from all six workouts entities. Columns named `template_id` exist on **three** entities (re-counted against a fresh metadata dump, correcting this ticket's original "two"): `WorkoutPlanAssignment.template_id`, `WorkoutSession.template_id`, `WorkoutTemplateExercise.template_id`. Plus `WorkoutTemplateExercise.weight_template`. **Proposal: not applied — see the investigation below.** The column name is agreed (`template_id`), but *which entity* is not settled by the evidence, and the three carriers mean three different things.
...
- **`date_range` — investigated, NOT applied, ruling proposed.** Every §6.1 and §6.2 row was read rather than counted. **It appears only in the Filters column, in six rows, and never in a Key Columns column**:
  ...
  - **Proposed ruling: drop `date_range` from the Finance row's Dimensions.** Evidence for it: (i) it is a filter in all six §6.1/§6.2 rows that declare it, so every report that needs it already has it in the right column and nothing is lost by removing it from §1.2; (ii) it names no column on any of the five finance entities; and (iii) a substitution is impossible **in principle**, not merely unclear — each §6.2 row's range binds to a different column (`Invoice.invoice_date`, the membership's signup date, `Payment`'s own date, `Refund.refund_date`), which is exactly why one name never fitted. If the intent was a specific report's date field, that belongs in that §6.2 row's Key Columns, not in §1.2.
  - **Why not applied**: the ruling removes a name rather than renaming one, so it is a different kind of edit from the other three and is worth a decision rather than a unilateral deletion.
- **`plan_template` — investigated, NOT applied, ruling proposed.** The column name is agreed; the entity is not.
  ...
  - **Proposed ruling: replace it with `template_id` and name the entity** — recommended `WorkoutSession.template_id` on the strength of the row's context and §6.4's established reporting key, with `WorkoutPlanAssignment.template_id` the alternative if the intent was the assignment. Not applied because the two candidates are equally real columns and choosing between them on the balance of evidence would be a guess presented as a fix.
- **Risks**: Four rows keep directing report authors at columns that do not exist. Two are now closed (`meal_template_id` named, `session_date` marked derived), and the remaining two are no longer unresearched — each carries findings and a proposal, with the `date_range` ruling deliberately left to a decision because it removes a name rather than renames one.
