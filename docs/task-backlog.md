...
- **Applied — resolution (B)**: §7.4 now states the boundary as a decision rather than a gap. The registry holds identity and last-refresh time; cadence, event trigger, debounce window and enabled/disabled stay in code/config permanently.
  ...
  - **Out-of-scope finding, deliberately not folded in**: §3.3 still says the table "does not exist in the database today" and that "no migration creates this table" (:276), (:289), and its own heading says "the migration still has to be written" (:274). `1788965263255-CreateMaterializedViewsTable.ts` creates it and the entity above maps to it, so those three statements are stale — a fourth instance of the pre-P6-01 staleness the note above P6-21 enumerated, which did not list §3.3. They concern the table's *existence*, not its cadence, so they are outside this ticket; recorded here because this ticket's verification surfaced them.
